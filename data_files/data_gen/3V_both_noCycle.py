# 3V systems, no cycle AKA cascade
...
# save_dir
save_dir=os.path.join(root, 'data_files', 'data', 'gen', '3V_both_noCycle')
...
# Y(t+1)=Y(t)*(Ry-Ry*Y(t)-Azy*Z(t)-Axy*X(t))
...
while(flag_rerun):
    ...
    for t in range(1, args.L):
        ...
        Y[t]=Y[t-1]*noiseY_mult*(Ry-Ry*Y[t-1]-Ayz*Z[t-1]-Axy*X[t-1])+noiseY_add
...
if args.noiseType!=None and args.noiseType.lower()!='none':
    data.to_csv(os.path.join(save_dir, f'3V_both_noCycle_{args.noiseType}_{args.noiseInjectType}_{args.noiseLevel}.csv'), index=False)
else:
    data.to_csv(os.path.join(save_dir, f'3V_both_noCycle_noNoise.csv'), index=False)