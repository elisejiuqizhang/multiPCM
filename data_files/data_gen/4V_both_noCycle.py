...
# save_dir
save_dir=os.path.join(root, 'data_files', 'data', 'gen', '4V_both_noCycle') 
...
if args.noiseType!=None and args.noiseType.lower()!='none':
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_{args.noiseType}_{args.noiseInjectType}_{args.noiseLevel}_1.csv'),index=False)
else:
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_noNoise_1.csv'),index=False)
...
# Y(t+1)=Y(t)*(Ry-Ry*Y(t)-Axy*X(t)-Awy*W(t)-Azy*Z(t))
...
while(flag_rerun):
    ...
    for t in range(1,args.L):
        ...
        Y[t]=Y[t-1]*noiseY_mult*(Ry-Ry*Y[t-1]-Axy*X[t-1]-Awy*W[t-1]-Azy*Z[t-1])+noiseY_add
...
data=pd.DataFrame({'X':X, 'Y':Y, 'W':W, 'Z':Z})
if args.noiseType!=None and args.noiseType.lower()!='none':
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_{args.noiseType}_{args.noiseInjectType}_{args.noiseLevel}_2.csv'),index=False)
else:
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_noNoise_2.csv'),index=False)
...
data=pd.DataFrame({'X':X, 'Y':Y, 'W':W, 'Z':Z})
if args.noiseType!=None and args.noiseType.lower()!='none':
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_{args.noiseType}_{args.noiseInjectType}_{args.noiseLevel}_3.csv'),index=False)
else:
    data.to_csv(os.path.join(save_dir,f'4V_both_noCycle_noNoise_3.csv'),index=False)
